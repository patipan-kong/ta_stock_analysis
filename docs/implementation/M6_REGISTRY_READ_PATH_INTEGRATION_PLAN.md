...
2. Wire it into Watchlist read paths only (`GET /watchlist`) as a display-enrichment addition (`display_symbol`, classification) — no behavior change to identity/dedup logic, purely additive response fields. **Shipped 2026-07-10** as the Watchlist Registry Pilot — see [WATCHLIST_REGISTRY_PILOT.md](WATCHLIST_REGISTRY_PILOT.md). The shipped shape differs slightly from this sketch: the additive field is `"registry": {resolved, asset_id, canonical_symbol, market, exchange}` (or `{resolved: false, reason}`), matching the shape Phase 3 step 7 already established for the Recommendation write path, rather than a separate `display_symbol`/classification-only projection — reused for consistency (ENGINEERING_PRINCIPLES.md "Shared Schemas"). `POST /watchlist` was wired identically, for response-shape consistency with `GET /watchlist` (both share `_watchlist_row()`), since it was not explicitly excluded and carries no additional risk.
3. Regression: full existing test suite green, plus new unit tests for `resolve_asset()` mirroring the M1–M4 testing idiom already established (in-memory SQLite, `_shape`/`_result` fixture helpers). **Done for the module itself** — 18 new tests plus the full pre-existing Asset Registry test family (133 tests total) green. **Step 2 done, 2026-07-10** — `backend/tests/test_watchlist_registry.py` (10 tests); full backend suite regression-verified via `git stash` isolation, byte-identical 58-failure pre-existing set before/after, 1063 → 1073 passed.
