...
def test_exists_is_false_for_bindings_with_no_definition():
    # M22: FUND is now defined; M24: BOND is now defined too; the remaining
    # ghosts are still undefined.
    registry = DefinitionRegistry.build()
...
def test_get_returns_governance_projection_or_none():
    ...
    assert registry.get("PROPERTY") is None  # M24: BOND is now defined; PROPERTY remains the undefined example
...
def test_all_returns_every_loaded_definition_sorted_by_binding():
    ...
    assert set(bindings) == {"BOND", "CASH", "EQUITY", "ETF", "FUND"}
